...
try:
    ...
    configFile = {"url": "https://raw.githubusercontent.com/ChickenDevs/webcast/main/test-off.xml", "videoDelay": 0,
                  "audioDelay": 1, "volume": 0, "preset": "veryfast"}
...
if system("ps -e | grep ffmpeg") == 0:  # ffmpeg is running
    running = True
    logger.info("Found ffmpeg running")
else:
    running = False
...
if shouldberunning and not running:
    ...
    system(f"ffmpeg -hide_banner   -f v4l2 -thread_queue_size 16 -framerate 25 "
           "-video_size 1280x720 -input_format mjpeg -i /dev/video0 "
           ...
           f"-ar {audiobitrate} -ac 2 -preset {config['preset']} -b:v {videobitrate} -framerate 25 "
